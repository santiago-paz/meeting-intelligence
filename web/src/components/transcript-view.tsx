    ...
    <ol className="divide-y divide-rule/60">
      {turns.map((turn) => (
        <li
          key={turn.idx}
          id={`turn-${turn.idx}`}
          className="turn grid scroll-mt-24 grid-cols-[5.5rem_1fr] gap-x-4 gap-y-1 rounded-sm px-2 py-3 sm:grid-cols-[6.5rem_1fr]"
          style={{ "--speaker": colors.get(turn.speaker) } as CSSProperties}
        >
          <a
            href={`#turn-${turn.idx}`}
            className="timecode row-span-2 pt-1 font-mono text-xs tabular-nums text-ink-muted hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink"
          >
